                    ...
                    <div className="flex-1 bg-zinc-950 p-8">
                        <div className="flex mb-6">
                            {["login", "signup"].map((index) => (
                                <button
                                    ...
                                    className={`text-sm md:text-lg pb-2 mr-6 border-b-2 transition-colors ${tab === index ? "text-blue-500 border-blue-500" : "text-zinc-500 border-transparent"}`}
                                >
                                    {index === "login" ? "Log in" : "Sign up"}
                                </button>
                            ))}
                        </div>
                        <button className="w-full flex justify-center items-center gap-2 border border-zinc-800 bg-zinc-900 text-zinc-200 text-sm md:text-lg py-2.5 rounded-lg hover:bg-zinc-700 hover:text-white hover:border-zinc-300 active:bg-zinc-900 active:border-zinc-800 transition-colors">
                        ...
                        {tab === "signup" && (
                            <div className="mb-3">
                                ...
                                <input type="text" placeholder="Your name" className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm md:text-base text-zinc-200 outline-none focus:border-blue-500 transition-colors"/>
                            </div>
                        )}
                        
                        <div className="mb-3">
                                <label className="text-xs md:text-base text-zinc-500 block mb-1 ml-1">Email address</label>
                                <input type="email" placeholder="you@example.com" className="w-full bg-zinc-900 border border-zinc-800 tracking-wide rounded-lg px-3 py-2 text-sm md:text-base text-zinc-200 outline-none focus:border-blue-500 transition-colors"/>
                            </div>

                            <div className="mb-4">
                                <label className="text-xs md:text-base text-zinc-500 block mb-1 ml-1">Password</label>
                                <input type="password" placeholder="••••••••" className="w-full bg-zinc-900 tracking-widest border border-zinc-800 rounded-lg px-3 py-2 text-sm md:text-base text-zinc-200 outline-none focus:border-blue-500 transition-colors"/>
                            </div>

                            <button className="w-full py-2.5 text-sm text-white md:text-base font-medium bg-blue-500  rounded-lg hover:bg-blue-600 transition-colors">
