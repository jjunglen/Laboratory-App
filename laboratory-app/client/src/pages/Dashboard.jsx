    ...
    const fetchImages = async () => {
      try {
        ...
        const res = await fetch(
          `https://thelabdtx.com/products/${handle}.json`,
        );
        if (res.ok) {
      ...
      <div className="relative bg-zinc-900 border border-zinc-800 rounded-t-2xl md:rounded-2xl w-full md:max-w-lg max-h-[92vh] flex flex-col z-10 overflow-hidden">
        ...
        <div className="overflow-y-auto flex-1">
          <div className="bg-white">
            {loadingImages ? (
              <div className="h-64 md:h-72 flex items-center justify-center bg-zinc-100">
              ...
              >
                <CarouselContent>
                  {images.map((img, i) => (
                    <CarouselItem key={i}>
                      <div className="h-64 md:h-72 flex items-center justify-center p-4">
                        <img
                          src={
                            img.includes("?")
                              ? `${img}&width=600`
            ...
            {images.length > 1 && (
              <div className="flex justify-center gap-1.5 pb-3">
                {images.map((_, i) => (
                  <div
                    key={i}
                    className={`rounded-full transition-all duration-200 ${i === activeIndex ? "w-3 h-1.5 bg-blue-500" : "w-1.5 h-1.5 bg-zinc-400"}`}
                  />
                ))}
              </div>
            )}
          ...
          <div className="p-5">
            <div className="flex gap-2 flex-wrap mb-3">
              {conditionLabel(shoe.condition) && (
                <span
                  className={`text-xs font-medium px-2.5 py-1 rounded-full ${shoe.condition === "brand_new" ? "bg-green-500 text-white" : "bg-zinc-700 text-zinc-200"}`}
                >
                  {conditionLabel(shoe.condition)}
                </span>
              )}
            ...
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                {shoe.compare_at_price &&
                  parseFloat(shoe.compare_at_price) >
                    parseFloat(shoe.price) && (
                    <p className="text-sm text-zinc-500 line-through">
                      ${parseFloat(shoe.compare_at_price).toFixed(0)}
                    </p>
                  )}
              </div>
              {shoe.compare_at_price &&
                parseFloat(shoe.compare_at_price) > parseFloat(shoe.price) && (
...
function ShoeCell({ shoe, mySizes, showTimeAgo = false }) {
    ...
    return (
      <div className="flex items-center gap-3 p-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-zinc-200 leading-tight line-clamp-2 mb-1">
            {shoe.shoe_name}
          </p>
          <div className="flex items-center gap-2 flex-wrap">
            <span
              className={`text-xs px-2 py-0.5 rounded-full font-medium ${mySizes?.includes(shoe.size) ? "bg-blue-500 text-white" : "bg-zinc-800 text-zinc-300"}`}
            >
              {shoe.size}
            </span>
            {showTimeAgo ? (
              ...
              conditionLabel(shoe.condition) && (
                <span
                  className={`text-xs px-2 py-0.5 rounded-full ${shoe.condition === "brand_new" ? "bg-green-500 text-white" : "bg-zinc-700 text-zinc-200"}`}
                >
                  {conditionLabel(shoe.condition)}
                </span>
              )
  ...
  return (
    <>
      ...
      <div className="p-3">
        <p className="text-xs md:text-sm font-medium text-zinc-200 leading-tight line-clamp-3 mb-2 min-h-[3.5rem]">
          {shoe.shoe_name}
        </p>
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-medium bg-blue-950 text-blue-300 px-2.5 py-1 rounded-lg">
            View →
          </span>
        </div>
      </div>
...
export default function Dashboard() {
  ...
  const inStockItems = shoeInventory.filter((item) =>
  ...
  const renderShoeCell = (shoe, showTimeAgo = false) => (
    <CellToggle
      key={shoe.id}
    ...
    >
      <ShoeCell shoe={shoe} mySizes={mySizes} showTimeAgo={showTimeAgo} />
    </CellToggle>
  );
      ...
      <div className="px-6 md:px-10 py-8 max-w-6xl mx-auto">
        ...
        {tab === "instock" && (
          <>
                  ...
                  <div className="flex gap-2 mt-1 flex-wrap">
                    {mySizes.length > 0 ? (
                      mySizes.map((size) => (
                        <span
                          key={size}
                          className="text-xs bg-zinc-900 border border-zinc-800 text-zinc-400 py-1 px-3 rounded-full"
                        >
                          {size}
                        </span>
                      ))
            ...
            {loadingInventory ? (
              ...
              <ContainerToggle>
                {inStockItems.map((shoe) => renderShoeCell(shoe))}
              </ContainerToggle>
            )}
          </>
        )}
        ...
        {tab === "new" && (
          <>
            ...
            {loadingInventory ? (
              ...
              <ContainerToggle>
                {newArrivals.map((shoe) => renderShoeCell(shoe, true))}
              </ContainerToggle>
            )}
          </>
        )}
        ...
        {tab === "browse" && (
          <>
            ...
            {loadingInventory ? (
              ...
              <ContainerToggle>
                {browseFiltered.map((shoe) => renderShoeCell(shoe))}
              </ContainerToggle>
            )}
          </>
        )}
        ...
        {tab === "alerts" && (
          <>
            ...
            {loadingAlerts ? (
              <p className="text-zinc-500 text-sm">Loading your alerts...</p>
            ) : alerts.length === 0 ? (
              <div className="text-center py-16">
                <p className="text-zinc-500 text-sm mb-4">No alerts yet</p>
                <button
                  onClick={() => navigate("/search")}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-zinc-500">{alert.size}</p>
                        {alert.max_price && (
                          <p className="text-xs text-zinc-600">
                            Max ${parseFloat(alert.max_price).toFixed(0)}
                          </p>
                        )}
                        <span className="inline-block mt-1 text-xs bg-blue-950 text-blue-300 px-2 py-0.5 rounded-full">
